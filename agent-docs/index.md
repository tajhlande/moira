...
| **goal-alignment-and-research-effectiveness.md** | Plan for fixing goal-alignment failures (5 of 7 questions scoring 2/5). Phased: Phase 1 = search strategy diversity (prompt-only, Change 3), Phase 2 = derivation core (Foundational + Changes 1+2, code+prompts), Phase 3 = report hedging (Change 4, optional). Tracks the agent's inability to answer questions when research is incomplete — catalogs gaps instead of synthesizing best-available answers. |
| **fact-extraction-and-claim-quality.md** | Plan for improving report quality across four areas: claim correction in research_review (reviewer provides corrected_claim for imprecise facts, flipping them to verified without retry), url_content underutilization (model relies on snippets, rarely fetches full pages), process-metadata claim detection via reviewer judgment (replaces brittle regex patterns with model-based detection — reviewer marks process-metadata claims as "unknown" to trigger retry), and citation curation for report generation (only verified-fact citations sent to the model, pre-numbered to eliminate ID translation errors). Based on investigation of the 2026-07-18 eval batch showing the citation chain is intact but fact extraction and claim precision are the bottleneck. |
| **recall-source-tool.md** | Plan for a built-in `recall_source` tool that lets the research model re-read the full stored content of previously-fetched citations during retry cycles. Follows the url_content dedup interception pattern — calls are intercepted in the research loop and synthesized from in-scope citations (no executor call, no budget charge). Always available as a default tool. |
