...
| Document | Description |
|---|---|
| **planning-freedom.md** | Experiment plan for the `planning-freedom` branch — replacing prescribed tool calls (`ToolCallPlan`) with `EvidenceRequest`s so planning specifies *what* evidence is needed and research decides *how* to retrieve it. Spike implemented; early evals are a diagnostic checkpoint on an unfinished attempt (fact discovery not yet improved — more work needed). Remaining phases fix extraction-fallback corruption, restore request-level traceability, add mechanical query dedup, wire planner/researcher evaluation dimensions into the eval harness, then a multi-batch adopt/iterate/rollback gate. |
