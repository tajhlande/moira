...
| Document | Description |
|---|---|
| **planning-freedom.md** | Experiment plan for the `planning-freedom` branch — replacing prescribed tool calls (`ToolCallPlan`) with `EvidenceRequest`s so planning specifies *what* evidence is needed and research decides *how* to retrieve it. Phases 0–3.3.2 complete: spike, Phase 2/2.1 extraction guards (eval-confirmed), Phase 3 request attribution + retry feedback (runtime-verified), Phase 3.1 store visibility + recall dedup, Phase 3.3 context budget (feedback cap, citation-limit retune to 5K with a single canonical constant, URL-field pruning — measured ~10x URL-density drop in model-facing windows). Remaining: Phase 4 mechanical query dedup, Phase 5 planner/researcher eval dimensions, Phase 6 multi-batch adopt/iterate/rollback gate (blocked on an eval-harness stale-checkpoint defect). Residual recall-overflow exposure tracked in-plan; deep reading of stored sources now owned by [summarize-source.md](summarize-source.md); retrieval noise by [retrieval-quality.md](retrieval-quality.md). |
...
| **retrieval-quality.md** | Pre-plan for the retrieval side of search: batch-to-batch eval noise is dominated by query-quality variance (evidence: same-question verified-fact swings, near-zero web_search cache hit rate, model's own "queries are returning irrelevant results"), not engine variance (Brave is paid/deterministic). Decomposes into vocabulary mismatch, sampling variance, and page-level luck. Proposals: retrieval-isolation harness (per-fact recall@k baseline), delegated query-writer pass, fact-type query templates sharing the source-type taxonomy, per-fact multi-query fan-out, passage-level retrieval (web_search overhaul), within-run feedback memory. Sequenced measurement-first. Not scheduled; separate stream from planning-freedom. |
| **hierarchical-decomposition.md** | Pre-plan / brainstorm for recursively decomposing hard questions into sub-question atoms researched independently, then assembled into an organized fact/conclusion base. Motivated by flat decomposition being a measured ceiling (median ~8 facts, research adds zero in 7/10 runs) and richness targets of 2-3 dozen organized facts. The merge boundary (how child conclusions compose into the parent) is the gating unsolved design problem. Staged Level 0 (prompt-only clustering) → Level 1 (map-reduce in one graph) → Level 2 (true recursion/subagents). Not scheduled; orthogonal to the planning-freedom branch. |
| **summarize-source.md** | Pre-plan for a `summarize_source` tool: a directed extraction call that reads an entire stored source in its own sub-context and returns salient facts (optionally focused on specific entities/topics) to the calling agent. Motivated by measured window starvation — feedback and citation-content caps make beyond-window data invisible, and `recall_source` can only re-inject the same window. Six open design decisions: two-tier storage vs re-fetch, provenance of extracted claims (unverified facts with quote anchors), summarizer model choice, execution shape (intercepted vs real tool), pricing in the primary budget currency (invocation_cost, no shadow budgets), and planner integration via `evidence_needed` as focus. Not scheduled; separate stream from planning-freedom. |
