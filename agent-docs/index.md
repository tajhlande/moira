...
| Document | Description |
|---|---|
| **planning-freedom.md** | Experiment plan for the `planning-freedom` branch — replacing prescribed tool calls (`ToolCallPlan`) with `EvidenceRequest`s so planning specifies *what* evidence is needed and research decides *how* to retrieve it. Spike implemented; Phase 2 defect fixes + Phase 2.1 overflow-split amendment code-complete — recoveries on telescope/jazz/tyranitar/cheetos in the 08-24/25 evals; remaining retrieval noise is tracked in [retrieval-quality.md](retrieval-quality.md). Remaining phases restore request-level traceability, add mechanical query dedup, wire planner/researcher evaluation dimensions into the eval harness, then a multi-batch adopt/iterate/rollback gate. |
